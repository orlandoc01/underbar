(function() {
  ...
  _.last = function(array, n) {
    ...
    var start = len - n >= 0 ? len - n : 0;
    return n === undefined ? array[len - 1] : array.slice(start, len);
  };
  ...
  _.uniq = function(array) {
    ...
    _.each(array, function(item) {
      if(_.indexOf(final,item) == -1) final.push(item);
    });
  ...
  _.extend = function(obj) {
    ...
    var finalObj = args.shift();
    _.each(args, function(obj) {
      _.each(obj, function(value, key) {finalObj[key] = value;});
    });
  ...
  _.defaults = function(obj) {
    ...
    _.each(args, function(obj) {
      _.each(obj, function(value, key) {
        if(!(key in finalObj)) finalObj[key] = value;
      });
    });
  ...
  _.delay = function(func, wait) {
    var n_args = Array.prototype.slice.call(arguments, 0);
    var func = n_args.shift();
    var wait = n_args.shift();
    setTimeout(function() { return func.apply(this, n_args); }, wait);
  ...
  _.shuffle = function(array) {
    ...
    for(var i = 0; i < array.length; i++) {
      choice = Math.floor(Math.random()*arrayCopy.length);
      finalArray.push(arrayCopy[choice]);
      arrayCopy.splice(choice, 1);
    }
